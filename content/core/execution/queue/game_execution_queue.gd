extends RefCounted
## Executes child gameplay operations in deterministic order with depth, budget, and cycle guards.
##
## The queue prevents recursive call chains and unsafe collection mutation. Limits are tracked
## per root operation, while error policy controls whether failures stop one root or all work.
class_name GameExecutionQueue

## Emitted after an operation is accepted into the queue.
signal operation_enqueued(operation: GameOperation)
## Emitted after an operation finishes execution.
signal operation_completed(operation: GameOperation, result: GameCommandResult)
## Emitted after all queued operations for a root chain are cancelled.
...
# ====== PUBLIC ========
## Configures root-chain guard limits and failure handling policy.
...
## Assigns the diagnostics sink used for queue traces and guard failures.
func set_diagnostics(diagnostics: GameDiagnosticsSink) -> void:
	_diagnostics = diagnostics

## Validates and appends [param operation] to the deterministic queue.
...
## Executes up to [param max_operations] queued operations, or all operations when negative.
...
## Cancels and removes queued operations belonging to [param root_operation_id].
...
## Cancels every queued root chain with [param reason].
...
## Cancels all work and resets queue bookkeeping.
func clear() -> void:
...
## Returns whether the queue is currently inside [method process].
func is_processing() -> bool:
	return _is_processing

## Returns the number of operations waiting to execute.
func get_pending_count() -> int:
	return _queue.size()

## Returns the current queue in deterministic execution order for diagnostics.
