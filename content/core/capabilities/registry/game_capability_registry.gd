extends RefCounted
## Stores and resolves capability providers for one local game object.
##
## The registry enforces declared cardinality, validates optional script contracts, and
## returns structured query results without searching the SceneTree.
...
# ====== PUBLIC ========
## Registers [param provider] for [param spec], enforcing the declared cardinality.
...
## Removes [param provider] from [param capability_id]. Returns whether it was registered.
...
## Resolves [param dependency] and validates cardinality and the optional script contract.
...
## Returns the first valid provider for [param capability_id], or [code]null[/code].
...
## Returns all currently valid providers for [param capability_id].
...
## Returns whether at least one valid provider exists for [param capability_id].
func has_capability(capability_id: StringName) -> bool:
	return not get_all(capability_id).is_empty()

## Returns the registered cardinality for [param capability_id].
...
## Removes every provider and cardinality registration.
func clear() -> void:
	_providers.clear()
	_cardinalities.clear()

## Returns a deterministic capability-to-provider snapshot for diagnostics.
